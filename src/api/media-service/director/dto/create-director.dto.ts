...
export class FilmographyItemDto {
    @ApiProperty({ enum: FilmographyRole, description: 'Role in the filmography' })
    @IsEnum(FilmographyRole)
    @IsNotEmpty()
    role: FilmographyRole;

    @ApiProperty({ description: 'Title of the movie or series' })
    @IsString()
    @IsNotEmpty()
    title: string;
}
...
export class CreateDirectorDto {
    ...
    @ApiPropertyOptional({ description: 'Filmography of the director', type: [Object] })
    @IsOptional()
    @IsArray()
